...
## Input Special
{% if "ssh" in linux_server_iptables_basic_services %}
# SSH
ipt46 -A INPUT -p tcp --dport 22 -j ACCEPT
{% endif %}
{% if "ntp" in linux_server_iptables_basic_services %}
# NTP
ipt46 -A INPUT -p udp --dport 123 -j ACCEPT
{% endif %}
{% if "prometheus-node-exporter" in linux_server_iptables_basic_services %}
